...
class GAPController extends Controller
{
        ...
        while ($parent) {
            ...
            if ($checkHasGAP && $gapSatisfyLevel) {
                ...
                if ($parent->save()) {
                    ...
                    $user_operation->operation_comment = sprintf('Группововй обьем %s в размере %s sv
    ...
    public function give_bonus($user_packet)
    {
        ...
        if (isset($inviter)) {
            $packet = Packet::where(['packet_id' => $user_packet->packet_id])->first();
            $bonus = $user_packet->packet_price * (15 / 100);
    ...
    public function give_status($user_packet)
    {
        ...
        try {
            ...
            $operation->operation_comment = 'Ваш GAP статус ' . $status;
            $operation->save();
