...
use App\Models\JasOtau;
use App\Models\Users;
use Illuminate\Http\Request;
use App\Http\Controllers\Controller;
use Illuminate\Support\Facades\DB;

class JasOtauController extends Controller
{
    public function index(Request $request)
    {
        $getGroup = JasOtau::orderBy('created_at', 'desc');
        if ($request->group_name){
            $getGroup = JasOtau::where('group_name', 'like', '%' . $request->group_name . '%');

        }
        $getGroup = $getGroup->paginate(15);
        return view('admin.shareholder-clients.jas-otau.cooperative-group',
            compact('getGroup'));
    }

    public function create()
    {
        $listOfProgram = CooperativeProgramm::where('program_code', '=', 8)->pluck('program_name', 'id')->toArray();
        return view('admin.shareholder-clients.jas-otau.create' , compact( 'listOfProgram'));
    }
    public function store(Request $request)
    {
        $date = $request->input();
        $item = (new JasOtau())->create($date);
        if ($item){
            return redirect()->route('jasotau.index')
    ...
    public function getUsers(Request $request){
        ...
        return view('admin.shareholder-clients.jas-otau.user_list', [
            'row' => $request,
    ...
    public function editUser($id){
        $user = Users::find($id);
        $listOfGroup = JasOtau::all()->pluck('group_name', 'id')->toArray();
        return view('admin.shareholder-clients.jas-otau.add_user_to_group', compact('user', 'listOfGroup'));
    }
    ...
    public function updateUser(Request $request, $id){
        ...
        if ($result){
            return redirect()->route('jasotau.index')->with('success', 'Пользователь успешно добавлен');
        }else{
            return back()->withErrors(['msg' => 'Error'])
                ->withInput();
        }
    ...
    public function getIdOfGroup($id){
        $users = Users::where('jas_otau_id', '=', $id)->paginate(5);
        if (isset($request->iin)){
            $users = Users::where('jas_otau_id', '=', $id)
                ->where('iin', 'like', '%'. $request->iin. '%');
        }


        return view('admin.shareholder-clients.jas-otau.auto_client', compact('users'));
    }
}
