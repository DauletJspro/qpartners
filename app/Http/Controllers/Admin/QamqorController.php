...
use App\Models\Qamqor;
use App\Models\Users;
use Illuminate\Http\Request;
use App\Http\Controllers\Controller;
use Illuminate\Support\Facades\DB;

class QamqorController extends Controller
{
    public function index(Request $request)
    {
        $getGroup = Qamqor::orderBy('created_at', 'desc');
        if ($request->group_name){
            $getGroup = Qamqor::where('group_name', 'like', '%' . $request->group_name . '%');

        }
        $getGroup = $getGroup->paginate(15);
        return view('admin.shareholder-clients.qamqor.cooperative-group',
            compact('getGroup'));
    }

    public function create()
    {
        $listOfProgram = CooperativeProgramm::where('program_code', '=', 6)->pluck('program_name', 'id')->toArray();
        return view('admin.shareholder-clients.qamqor.create' , compact( 'listOfProgram'));
    }
    public function store(Request $request)
    {
        $date = $request->input();
    $item = (new Qamqor())->create($date);
        if ($item){
            return redirect()->route('qamqor.index')
    ...
    public function getUsers(Request $request){
        $request->row = Users::leftJoin('city', 'city.city_id', '=', 'users.city_id')
            ->where('packet.packet_id','=', 41)
            ->where('users.qamqor_id', '=' , 0)
        ...
        return view('admin.shareholder-clients.qamqor.user_list', [
            'row' => $request,
    ...
    public function editUser($id){
        $user = Users::find($id);
        $listOfGroup = Qamqor::all()->pluck('group_name', 'id')->toArray();
        return view('admin.shareholder-clients.qamqor.add_user_to_group', compact('user', 'listOfGroup'));
    }
    ...
    public function updateUser(Request $request, $id){
        ...
        if ($result){
            return redirect()->route('qamqor.index')->with('success', 'Пользователь успешно добавлен');
        }else{
            return back()->withErrors(['msg' => 'Error'])
                ->withInput();
        }
    ...
    public function getIdOfGroup($id){
        $users = Users::where('qamqor_id', '=', $id)->paginate(5);
        if (isset($request->iin)){
            $users = Users::where('qamqor_id', '=', $id)
                ->where('iin', 'like', '%'. $request->iin. '%');
        }


        return view('admin.shareholder-clients.qamqor.auto_client', compact('users'));
    }
}
