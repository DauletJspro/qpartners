...
use App\Models\Qoldau;
use App\Models\Users;
use Illuminate\Http\Request;
use App\Http\Controllers\Controller;
use Illuminate\Support\Facades\DB;

class QoldauController extends Controller
{
    public function index(Request $request)
    {
        $getGroup = Qoldau::orderBy('created_at', 'desc');
        if ($request->group_name){
            $getGroup = Qoldau::where('group_name', 'like', '%' . $request->group_name . '%');

        }
        $getGroup = $getGroup->paginate(15);
        return view('admin.shareholder-clients.qoldau.cooperative-group',
            compact('getGroup'));
    }

    public function create()
    {
        $listOfProgram = CooperativeProgramm::where('program_code', '=', 5)->pluck('program_name', 'id')->toArray();
        return view('admin.shareholder-clients.qoldau.create' , compact( 'listOfProgram'));
    }
    public function store(Request $request)
    {
        $date = $request->input();
        $item = (new Qoldau())->create($date);
        if ($item){
            return redirect()->route('qoldau.index')
    ...
    public function getUsers(Request $request){
        $request->row = Users::leftJoin('city', 'city.city_id', '=', 'users.city_id')
            ->where('packet.packet_id','=', 43)
            ->where('users.qoldau_id', '=' , 0)
        ...
        return view('admin.shareholder-clients.qoldau.user_list', [
            'row' => $request,
    ...
    public function editUser($id){
        $user = Users::find($id);
        $listOfGroup = Qoldau::all()->pluck('group_name', 'id')->toArray();
        return view('admin.shareholder-clients.qoldau.add_user_to_group', compact('user', 'listOfGroup'));
    }
    ...
    public function updateUser(Request $request, $id){
        ...
        if ($result){
            return redirect()->route('qoldau.index')->with('success', 'Пользователь успешно добавлен');
        }else{
            return back()->withErrors(['msg' => 'Error'])
                ->withInput();
        }
    ...
    public function getIdOfGroup($id){
        $users = Users::where('qoldau_id', '=', $id)->paginate(5);
        if (isset($request->iin)){
            $users = Users::where('qoldau_id', '=', $id)
                ->where('iin', 'like', '%'. $request->iin. '%');
        }


        return view('admin.shareholder-clients.qoldau.auto_client', compact('users'));
    }
