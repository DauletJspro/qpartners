...
class ShareHolder2Controller extends Controller
{
    ...
    public function index(Request $request)
    {
        $request->row = Users::leftJoin('city','city.city_id','=','users.city_id')
            ...
            ->orderBy('users.user_share2','desc')
        ...
        $row = UserOperation::leftJoin('users','users.user_id','=','user_operation.author_id')
            ...
            ->where('user_operation.operation_type_id',25)
        ...
        $request->shareholder_profit_today = UserOperation::where('operation_type_id',25)
        ...
        return  view('admin.shareholder2.shareholder2',[
            'row' => $request,
            'title' => 'Дольщики',
            'request' => $request
        ]);
    ...
    public function showAddUserShare()
    {
        ...
        return  view('admin.shareholder2.shareholder2-add', [
            'title' => 'Добавить долю',
            'row' => $row
        ]);
    }
    ...
    public function addUserShare(Request $request)
    {
        ...
        $operation->operation_type_id = 23;
        $operation->operation_comment = $request->operation_comment;;
        $operation->save();

        $user = Users::where('user_id',$request->user_id)->first();
        $user->user_share2 = $user->user_share2 + $request->user_share;
        $user->save();

        return redirect('/admin/shareholder2');
    }
        ...
        if($fond->fond_money > 0){
            ...
            foreach ($shareholder_list as $key => $item){
                ...
                if($operation_check == 0){
                    ...
                    $operation->money = $unit_share * $item->user_share2;
                    $operation->operation_id = 1;
                    $operation->operation_type_id = 24;
                    $operation->operation_comment = round($unit_share,2) ." $ x ".$item->user_share2 ." доля";
                    $operation->save();

                    $user = Users::where('user_id',$item->user_id)->where('is_activated',1)->first();
                    if($user == null) continue;
                    $user->user_money = $user->user_money + ($unit_share * $user->user_share2);
                    $user->save();
                }
            }
            $fond->fond_money = 0;
