...
<head>
    ...
    <link href="/admin/css/admin.css?v=1" rel="stylesheet" type="text/css" />
        ...
        <div class="body bg-gray">
            <div class="row">
                ...
                <div class="col-md-4">
                    ...
                    <div class="form-group">
                        <select required name="speaker_id" data-placeholder="Выберите спикера" class="form-control selectpicker" data-live-search="true">
                ...
                <div class="col-md-4">
                    ...
                    <div class="form-group">
                        <select id="city_id" required name="city_id" data-placeholder="Выберите город" class="form-control" data-live-search="true">
                            <option value="">Выберите город (Юридический адрес)</option>
                    ...
                    <div class="form-group">
                        <input required type="text" name="address" value="{{$row->address}}" class="form-control" placeholder="Район, Юридический адрес"/>
                    </div>
                    <div class="form-group">
                        <select onchange="getCityListByCountry2(this)" required name="fact_country_id" data-placeholder="Выберите страну" class="form-control" data-live-search="true">
                            <option value="">Выберите страну</option>
                            @foreach($country_row as $item)
                                <option @if($row->country_id == $item->country_id) {{'selected'}} @endif value="{{$item->country_id}}">{{$item['country_name_ru']}}</option>
                            @endforeach
                    ...
                    <div class="form-group">
                        <select id="fact_city_id" required name="fact_city_id" data-placeholder="Выберите город" class="form-control" data-live-search="true">
                            <option value="">Выберите город (Фактический адрес)</option>
                            @foreach($city_row as $item)
                                <option @if($row->city_id == $item->city_id) {{'selected'}} @endif value="{{$item->city_id}}">{{$item['city_name_ru']}}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        <input required type="text" name="fact_address" value="{{$row->fact_address}}" class="form-control" placeholder="Район, Адрес получение посылки от Холдинга"/>
                    </div>
                </div>
                <div class="col-md-12 agree-label">
                    Регистрируясь на сайте вы подтверждаете, что ознакомлены с <a target="_blank" style="font-weight: bold" href="/file/dogovor_QT.pdf">Договором</a> и <a target="_blank" style="font-weight: bold" href="/presentation/qaz.pdf">презентацией</a>.
                </div>
            </div>
        </div>
        <div class="footer">
