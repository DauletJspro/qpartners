            ...
            <div class="col-md-12">
                <div class="box box-primary document-list">
                    ...
                    <form id="document_upload_form" enctype="multipart/form-data" method="post">
                        <div id="document_list" style="display: flex;  flex-wrap: wrap; max-width: 644px; justify-content: space-between">
                            @if(isset($document_list))

                                @foreach($document_list as $key => $item)

                                    <div class="item-docs" id="document_item_{{$item->document_id}}">
                                        <span class="document-name">{{$item->document_name_ru}}@if($item->is_required == 1)</br>
                                            <span style="font-weight: bold; color: red">(обязательно)</span>@endif</span>
                                        <div class="input-box clearfix">
                                                ...
                                                @foreach($document_request as $document_item)
                                                    ...
                                                    <div class="i-docs">
                                                        ...
                                                        @if($document_item->document_type == 'image')
                                                            ...
                                                            <a target="_blank"
                                                               href="{{$document_item['document_url']}}" {{--rel="group" title="1" class="gallery"--}}>
                                                                <img class="img-100"
                                                                     src="{{$document_item['document_url']}}"
                                                                     name="request_document[]">
                                                            </a>

                                                        @elseif(in_array($document_item->document_type,['doc','xlsx','pdf','csv','ppt']))

                                                            <a href="{{$document_item['document_url']}}"
                                                               target="_blank">
                                                                <img class="img-100"
                                                                     src="{{'/custom/image/'.$document_item->document_type.'.png'}}"
                                                                     name="request_document[]">
                                                            </a>
                                                        ...
                                                        @else
                                                            ...
                                                            <a href="{{$document_item['document_url']}}"
                                                               target="_blank">
                                                                <img class="img-100" src="/custom/image/ic_files.png"
                                                                     name="request_document[]">
                                                            </a>

                                                        @endif

                                                        <input type="hidden" value="{{$document_item['document_url']}}"
                                                               class="request-document"/>
                                                        <input type="hidden" value="{{$document_item['document_type']}}"
                                                               class="document-type"/>
                                                        <input type="hidden" value="{{$document_item['document_id']}}"
                                                               class="document-id"/>


                                                        <a href="javascript:void(0)"
                                                           onclick="deleteServiceDocument(this)"><i
                                                                    class="icons ic-del"></i></a>
                                            ...
                                            <div class="i-docs">
                                                <input type="file" name="image_{{$item->document_id}}"
                                                       onchange="uploadServiceDocument(this,'{{$item->document_id}}')">
